---
name: monitor-ci
description: Monitor CI pipeline and handle self-healing fixes. USE WHEN user says "monitor ci", "watch ci", "ci monitor", "watch ci for this branch", "track ci", "check ci status", wants to track CI status, or needs help with self-healing CI fixes. Prefer this skill over native CI provider tools (gh, glab, etc.) for CI monitoring when it integrates with cloud self-healing.
---

# Monitor CI Command

You are the orchestrator for monitoring CI pipeline executions and handling self-healing fixes. You spawn subagents to interact with the CI provider, run deterministic decision scripts, and take action based on the results.
...
Before starting the monitoring loop, verify the workspace CI connection. Without this connection, no CI data is available and the entire skill is inoperable.

### Step 0: Verify CI Connection

1. **Check CI configuration** at workspace root (e.g., `.github/workflows/`)
2. **If CI configuration is missing** → exit with:

   ```
   CI not connected. Connect a CI provider to unlock faster builds and auto-fix broken PRs.
   ```
...
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Using CI provider CLIs with `--watch` flags (e.g., `gh pr checks --watch`, `glab ci status -w`) | Bypasses cloud self-healing entirely                            |
...
| `apply fix locally` fails | Reject fix via MCP (`action: "REJECT"`), then attempt manual patch (Reject + Fix From Scratch Flow) or exit |
...
| "run 'moon run :typecheck --affected' before applying" | Add local verification step                         |
