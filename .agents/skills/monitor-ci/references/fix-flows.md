...
1. Report the skip reason to the user (e.g., "Auto-apply was skipped because the previous CI pipeline execution was triggered by cloud self-healing")
...
1. **Detect package manager:** `pnpm-lock.yaml` → `pnpm moon`, `yarn.lock` → `yarn moon`, otherwise `npx moon`
...
1. `apply fix locally` (sets state to `APPLIED_LOCALLY`)
